...
string suser = "";
string spass = "";
string server_ip = "20.150.196.196";
int configPort = 0;
string sdatabase = "";
...
public:
	...
	void Insert(string a) {
		...
		data[len] = index;
		data[len + 1] = 0x2;
		size_t str_len = a.length(); // resign size_t to int.
...
{
	const size_t packetLenght = packet->dataLength;
	BYTE* result = nullptr;
...
inline void removeExtraSpaces(string& str)
{
	try {
		...
		while (j < n)
		{
			if (str.at(j) != ' ')
			{
				if ((str.at(j) == '.' || str.at(j) == ',' || str.at(j) == '!' || str.at(j) == '?') && i - 1 >= 0 && str.at(i - 1) == ' ')
					str.at(i - 1) = str.at(j++);
				else
					str.at(i++) = str.at(j++);
				spaceFound = false;
...
inline unsigned char* getA(const string fileName, int* pSizeOut, bool bAddBasePath, bool bAutoDecompress)
{
	...
	if (!fp)
	{
		ColoredText("File not found (getA)", "red");
...
public:
	...
	void addButtonIcon(string buttonname, string text, int iconid);
...
inline void GTDialog::addButtonIcon(const string buttonname, const string text, const int iconid)
{
	this->dialogstr.append("add_checkicon|" + buttonname + "|" + text + "||" + to_string(iconid) + "||0|\n");
}
...
{
	this->dialogstr.append("text_scaling_string||\nset_default_color|\n");
}
