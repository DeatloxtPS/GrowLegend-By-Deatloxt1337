...
static int sb_buffer_push_char(sb_Buffer *buf, char chr) {
  if (buf->len == buf->cap) {
	  const int err = sb_buffer_reserve(buf, (buf->cap << 1) | (!buf->cap));
    if (err) return err;
  }
  buf->s[buf->len++] = chr;
  return SB_ESUCCESS;
...
static int sb_buffer_null_terminate(sb_Buffer *buf) {
	const int err = sb_buffer_push_char(buf, '\0');
  if (err) return err;
  buf->len--;
  return SB_ESUCCESS;
}
...
void sb_stream_destroy(sb_Stream *st) {
  sb_Event e;
  /* Emit close event */
  e.type = SB_EV_CLOSE;
  sb_stream_emit(st, &e);
...
send_data:
    ...
    const int err = sb_buffer_reserve(&st->send_buf, 8192);
    if (err) return err;
...
int sb_write(sb_Stream *st, const void *data, size_t len) {
  if (st->state < STATE_SENDING_DATA) {
	  const int err = sb_stream_finalize_header(st);
    if (err) return err;
  }
...
sb_Server *sb_new_server(const sb_Options *opt) {
  ...
  size_t err, optval; // Warning 1047_

#ifdef _WIN32
  { WSADATA dat; (void)WSAStartup(MAKEWORD(2, 2), &dat); }
#else
