...
string header_server_ip = "";
int header_configPort = 0;
...
uint8_t* read_file(const char* file, uint32_t* size) {
	...
	if (!fp) 
	{
		...
		ColoredText("Missing File: " + file_name + "", "red");
		ofstream error404("config+/log/error_404.txt", ios::app);
		error404 << "Missing File: " + file_name << endl;
		error404.close();
		return nullptr;
	}
	fseek(fp, 0, SEEK_END);
	...
	if (evt->type == SB_EV_REQUEST) {
		...
		if (strstr(evt->path, "/growtopia/server_data.php") != nullptr && request == "POST") {
			try {
				...
				ok:
					if (cooldown.find(address) == cooldown.end()) {
						cooldown.insert({ address, (GetCurrentTimeInternalSeconds() + (10)) });
					}
					else {
						cooldown.at(address) = (GetCurrentTimeInternalSeconds() + (10));
					}
					memcpy(reply, info.c_str(), info.size() + 1);
					sb_writef(evt->stream, format(reply, http_ip.c_str(), http_port.c_str(), http_ip.c_str()).c_str());
				}
...
void http::run(int port, string ip, int w_port) {
	...
	if (!http_server) {
		ColoredText("HTTP failed to start service, is " + http_ip + ":" + http_port + " already running?", "red");
		system("PAUSE");
		exit(0);
	}
	else {
