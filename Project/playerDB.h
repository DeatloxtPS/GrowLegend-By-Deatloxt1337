...
struct PlayerInfo {
	...
	long long int lastdrop = 0;
	long long int lastdealchange = 0;
	long long int lastSPIN = 0;
	long long int lastDM = 0;
	long long int lastFW = 0;
	long long int lastEff = 0;
	long long int lastkickall = 0;
	...
	if (peer) {
		if (a1 == 4 && *(static_cast<BYTE*>(packetData) + 12) & 8) {
			...
			try {
				...
				ColoredText("Failed to send packet raw (sendpacketraw)", "yellow");
			}
		}
	}
...
inline void updateplayerset(ENetPeer* peer, int targetitem) {
	...
	if (clothmask == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		...
		if (!iscontains)
		{
			...
			sendClothes(peer);
			static_cast<PlayerInfo*>(peer->data)->effect;
	...
	if (clothitem == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		auto iscontains = false;
		SearchInventoryItem(peer, item, 1, iscontains);
		if (!iscontains)
	...
	if (clothface == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		auto iscontains = false;
		SearchInventoryItem(peer, item, 1, iscontains);
		if (!iscontains)
	...
	if (clothneck == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		auto iscontains = false;
		SearchInventoryItem(peer, item, 1, iscontains);
		if (!iscontains)
	...
	if (clothshirt == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		auto iscontains = false;
		SearchInventoryItem(peer, item, 1, iscontains);
		if (!iscontains)
	...
	if (clothback == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		auto iscontains = false;
		SearchInventoryItem(peer, item, 1, iscontains);
		if (!iscontains)
	...
	if (clothances == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		auto iscontains = false;
		SearchInventoryItem(peer, item, 1, iscontains);
		if (!iscontains)
	...
	if (clothpants == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		auto iscontains = false;
		SearchInventoryItem(peer, item, 1, iscontains);
		if (!iscontains)
	...
	if (clothfeet == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		auto iscontains = false;
		SearchInventoryItem(peer, item, 1, iscontains);
		if (!iscontains)
	...
	if (clothhair == item && item != 0 && static_cast<PlayerInfo*>(peer->data)->effect != 8421376)
	{
		auto iscontains = false;
		SearchInventoryItem(peer, item, 1, iscontains);
		if (!iscontains)
...
inline bool EventWorld(ENetPeer* peer2, string WorldName) {
	if (static_cast<PlayerInfo*>(peer2->data) == NULL || peer2->data == NULL) {
		ColoredText("Playerinfo was nullptr", "yellow");
		return "START";
	}
...
inline void save_playerinfo(PlayerInfo* pData) {
	...
	try {
		...
		if (pData->needsaveinventory) {
			ifstream read_entry("config+/register/starting_items.json");
			json jxx;
			read_entry >> jxx;
			read_entry.close();
			int endpoint = jxx["endpoint"];
			json jjall = json::array();
			json jj;
			for (int i = 0; i < endpoint; i++) { // helps pervent store crash.
...
inline int PlayerDB::playerRegister(ENetPeer* peer, string username, string password, string email) {
	...
	auto existsRegisteringIP = std::experimental::filesystem::exists("save/registeredIP/" + static_cast<PlayerInfo*>(peer->data)->charIP + ".txt");
	if (existsRegisteringIP) {
		ifstream getIp("save/registeredIP/" + static_cast<PlayerInfo*>(peer->data)->charIP + ".txt");
		getIp >> registeringIpCount;
		registeringIpCount += 1;
		getIp.close();
		ifstream read_entryxj("config+/register/acc_create_limit.json");
		json jxj;
		read_entryxj >> jxj;
		read_entryxj.close();
		int createlimit = jxj;
		if (registeringIpCount > createlimit) {
			static_cast<PlayerInfo*>(peer->data)->maxipcheck = true;
		}
		if (registeringIpCount > createlimit) return -6;
		ofstream writeIp("save/registeredIP/" + static_cast<PlayerInfo*>(peer->data)->charIP + ".txt");
	...
	if (!existGems) {
		...
		int slot7 = jxx["slot7"];
		int slot8 = jxx["slot8"];
		int slot3amount = jxx["slot3amount"];
		int slot4amount = jxx["slot4amount"];
		int slot5amount = jxx["slot5amount"];
		int slot6amount = jxx["slot6amount"];
		int slot7amount = jxx["slot7amount"];
		int slot8amount = jxx["slot8amount"];
		int startpoint = jxx["startpoint"];
		int endpoint = jxx["endpoint"];
		json items;
		json jjall = json::array();
		json jj;
		...
		jj["aposition"] = 3;
		jj["itemid"] = slot3;
		jj["quantity"] = slot3amount;
		jjall.push_back(jj);
		jj["aposition"] = 4;
		jj["itemid"] = slot4;
		jj["quantity"] = slot4amount;
		jjall.push_back(jj);
		jj["aposition"] = 5;
		jj["itemid"] = slot5;
		jj["quantity"] = slot5amount;
		jjall.push_back(jj);
		jj["aposition"] = 6;
		jj["itemid"] = slot6;
		jj["quantity"] = slot6amount;
		jjall.push_back(jj);
		jj["aposition"] = 7;
		jj["itemid"] = slot7;
		jj["quantity"] = slot7amount;
		jjall.push_back(jj);
		jj["aposition"] = 8;
		jj["itemid"] = slot8;
		jj["quantity"] = slot8amount;
		jjall.push_back(jj);
		...
		json j;
		json jx;
		ifstream read_entryx("config+/register/inventorysize.json");
		read_entryx >> jx;
		read_entryx.close();
		int starting_inventory = jx;
		j["username"] = username;
		j["password"] = password;
		...
		j["notebook"] = "";
		j["kills"] = 0;
		j["inventorysize"] = starting_inventory;
		j["dailyrewards"] = 0;
	...
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		...
		if (peer != currentPeer) {
			if (isHere(peer, currentPeer)) {
				...
				//				if (static_cast<PlayerInfo*>(peer->data)->adminLevel = 5) peer_smstate = 1;
				if (static_cast<PlayerInfo*>(peer->data)->isinv) peer_is_invis = 1;
				int currentpeer_smstate = 0, currentpeer_is_invis = 0;
				//				if (static_cast<PlayerInfo*>(currentPeer->data)->adminLevel = 5) currentpeer_smstate = 1;
...
std::string currentISO8601TimeUTC() {
	...
	ss << std::put_time(gmtime(&itt), "");
	return ss.str();
}
inline void sendTime(ENetPeer* peer)
{
	Player::OnConsoleMessage(peer, "GTLG Time: " + currentISO8601TimeUTC());
}
...
inline void SendRegisterDialog(ENetPeer* peer) {
	if (static_cast<PlayerInfo*>(peer->data)->maxipcheck) {
		Player::OnTalkBubble(peer, static_cast<PlayerInfo*>(peer->data)->netID, " `oYou reached your account limit for this IP address!", 0, true);
		return;
	}
	else
	{
