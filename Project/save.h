...
inline void save_manager()
{
	while (true)
	{
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			if (static_cast<PlayerInfo*>(currentPeer->data)->isSBcooldown) {
				auto cooldownleft = calcBanDuration(static_cast<PlayerInfo*>(currentPeer->data)->lastSB);
				if (cooldownleft < 1) {
					Player::OnConsoleMessage(currentPeer, "You can /sb again! (`$SB cooldown`` mod removed)");
					static_cast<PlayerInfo*>(currentPeer->data)->isSBcooldown = false;
					static_cast<PlayerInfo*>(currentPeer->data)->lastSB = 0;
				}
			}
