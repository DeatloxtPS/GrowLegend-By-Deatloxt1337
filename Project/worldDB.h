...
inline int getPlayersCountInWorldEvent(WorldInfo* world, string name)
{
	...
	try
	{
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer)
		{
			...
			{
				ColoredText("currentPeer was nullptr", "yellow");
				continue;
			}
...
inline void buildItemsDatabase() {
	...
	for (string line; getline(infile, line);) {
		if (line.length() > 8 && line[0] != '/' && line[1] != '/') {
			...
			if (++current != def.id)
			{
				ColoredText("Unordered database at item " + std::to_string(current) + "/" + std::to_string(def.id), "red");
			}
			maxItems = def.id;
...
inline void restoreplayernick(ENetPeer* peer)
{
	...
	if (world == NULL) {
		ColoredText(" problem restoring nickname", "yellow");
		return;
	}
	try {
...
inline int processTakeServer(WorldInfo* world, ENetPeer* peer, const int itemIdx) {
	//WorldInfo* world = getPlyersWorld(peer);
	if (world == NULL) {
		ColoredText("processTakeServer error", "red");
		return -1;
	}
	if (!world) return -1;
	try {
...
inline void SendXP(ENetPeer* peer, int amount) {
	...
	if (isYellowAnces(GetPeerData(peer)) || FarmingEvent)
	{
		...
		else if (static_cast<PlayerInfo*>(peer->data)->cloth_ances == 5126)
		{
			const auto chance = (rand() % 100) + 1;
			if (chance <= 10)
			{
		...
		else if (static_cast<PlayerInfo*>(peer->data)->cloth_ances == 5128)
		{
			const auto chance = (rand() % 100) + 1;
			if (chance <= 15)
			{
		...
		else if (static_cast<PlayerInfo*>(peer->data)->cloth_ances == 5130)
		{
			const auto chance = (rand() % 100) + 1;
			if (chance <= 20)
			{
		...
		else if (static_cast<PlayerInfo*>(peer->data)->cloth_ances == 5132)
		{
			const auto chance = (rand() % 100) + 1;
			if (chance <= 25)
			{
		...
		else if (static_cast<PlayerInfo*>(peer->data)->cloth_ances == 5134)
		{
			const auto chance = (rand() % 100) + 1;
			if (chance <= 30)
			{
...
inline void DropGem(ENetPeer* peer, const int x, const int y, WorldInfo* world, int kiek = 0) {
	try {
		auto b = getGemCount(world->items.at(x + (y * world->width)).foreground) + rand() % 1;
		...
		while (b > 0) {
			...
			if (b >= 1) {
				...
				b -= 1;
				for (auto i = 0; i < rand() % 1; i++) DropItem(world, peer, -1, x * 32 + (rand() % 8), y * 32 + (rand() % 8), 112, 1, 0);
				continue;
			}
		}
	}
...
inline void SendCombiner(ENetPeer* peer, WorldInfo* world, const int x, const int y) {
	try {
		...
		ifstream infile("config+/binary.txt");
		for (string line; getline(infile, line);) {
...
inline void SendWeather(ENetPeer* peer, int tile, WorldInfo* world, int x, int y)
{
	try {
		...
		}
		{
			if (tile == 18)
			{
				if (world->items.at(x + (y * world->width)).foreground == 6854) // digital rain
				{
					if (world->weather == 42)
					{
						world->weather = 0;
						world->items.at(x + (static_cast<std::vector<WorldItem, std::allocator<WorldItem>>::size_type>(y) * world->width)).activated = false;
					}
					else
					{
						world->weather = 42;
						world->items.at(x + (static_cast<std::vector<WorldItem, std::allocator<WorldItem>>::size_type>(y) * world->width)).activated = true;
					}
					ENetPeer* currentPeer;
					for (currentPeer = server->peers;
						currentPeer < &server->peers[server->peerCount];
		...
		}
		{
			if (tile == 18)
			{
				if (world->items.at(x + (static_cast<std::vector<WorldItem, std::allocator<WorldItem>>::size_type>(y) * world->width)).foreground == 5654) // Valentines
				{
					if (world->weather == 36)
					{
						world->weather = 0;
		...
		}
		{
			if (tile == 18)
			{
				if (world->items.at(x + (static_cast<std::vector<WorldItem, std::allocator<WorldItem>>::size_type>(y) * world->width)).foreground == 5716) // St. Paddy's Day
				{
					if (world->weather == 37)
					{
						world->weather = 0;
		...
		}
		{
			if (tile == 18)
			{
				if (world->items.at(x + (y * world->width)).foreground == 4892) // balloon weather
				{
					if (world->weather == 33)
					{
						world->weather = 0;
		...
		}
		{
			if (tile == 18)
			{
				if (world->items.at(x + (y * world->width)).foreground == 8896) // Meteor shower
				{
					if (world->weather == 47)
					{
						world->weather = 0;
...
inline void restore_prices() {
	try {
		char oldname[] = "config+/price_data.txt";
		char newname[] = "config+/price_data1.txt";
		char oldname1[] = "config+/price_data1.txt";
		char newname1[] = "config+/price_data2.txt";
		char oldname2[] = "config+/price_data2.txt";
		char newname2[] = "config+/price_data3.txt";
		auto itemid = 0;
		auto kaina = 0;
		string append = "";
		int drop_k = 0;
		...
		remove("config+/price_data3.txt");
		if (rename(oldname2, newname2) != 0) {
			ColoredText("Filesystem Exception #3 Failed to rename file", "red");
		}
		remove("config+/price_data2.txt");
		if (rename(oldname1, newname1) != 0) {
			ColoredText("Filesystem Exception #2 Failed to rename file", "red");
		}
		remove("config+/price_data1.txt");
		if (rename(oldname, newname) != 0) {
			ColoredText("Filesystem Exception #1 Failed to rename file", "red");
		}
		remove("config+/price_data.txt");
		ofstream breaklogs("config+/price_data.txt", ios::app);
		breaklogs << append;
		breaklogs.close();
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			if (currentPeer->state != ENET_PEER_STATE_CONNECTED || currentPeer->data == NULL) continue;
		...
		ColoredText("Item prices were refreshed", "yellow");
	}
	catch (const std::out_of_range& e) {
		std::cout << e.what() << std::endl;
	}
...
inline void apply_lock_packet(WorldInfo* world, ENetPeer* peer, int x, int y, int id, int lockowner) {
	...
	int locksize = 0;
	if (id == 202) locksize = 25; // Lock Size
	PlayerMoving pmov{};
	pmov.packetType = 0xf;
	pmov.characterState = 0;
	pmov.x = 0;
...
inline void SendPunishView(ENetPeer* peer, string PlayerName) {
	...
	try
	{
		...
		GTDialog allinfo;
		allinfo.addCustom("add_label|mid|`5Punish/View: `oUsername: `0" + username + "`o, Nick: `o(``" + nick + "`o),  ID: " + to_string(playerid) + "|right\n");
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			if (static_cast<PlayerInfo*>(currentPeer->data)->rawName == username || static_cast<PlayerInfo*>(currentPeer->data)->charIP == ip || static_cast<PlayerInfo*>(currentPeer->data)->email == email) {
				...
				allinfo.addButton("warpto_" + static_cast<PlayerInfo*>(currentPeer->data)->currentWorld + "", "`oWarp to " + username + " (in `5" + static_cast<PlayerInfo*>(currentPeer->data)->currentWorld + "`o)");
		...
		allinfo.addLabelWithIconButton("View Inventory: ", 9412, "showuserinventoryoffline_" + username + "");
		allinfo.addLabelWithIconButton("Game Ban: ", 732, "banpanel");
		allinfo.addLabelWithIconButton("Duct-tape: ", 408, "mutepanel");
		allinfo.addLabelWithIconButton("Curse: ", 278, "cursepanel");
		allinfo.addSpacer(SPACER_SMALL);
...
inline void SendGrowpedia(ENetPeer* peer) {
	...
	if (static_cast<PlayerInfo*>(peer->data)->level >= 1) {
		level1 = "`oUnlocked: 2500 Gems``";
	}
	else {
		level1 = "`a(Locked) 2500 Gems``";
	}
	if (static_cast<PlayerInfo*>(peer->data)->level >= 2) {
		level2 = "`oUnlocked: 150 Chandelier``";
	}
	else {
		level2 = "`a(Locked) 150 Chandelier``";
	}
	...
	if (static_cast<PlayerInfo*>(peer->data)->level >= 4) {
		level4 = "`oUnlocked: Earth Mastery (Level 1) - 2% chance to break dirt in a single hit``";
	}
	else {
		level4 = "`a(Locked) Earth Mastery (Level 1) - 2% chance to break dirt in a single hit``";
	}
	if (static_cast<PlayerInfo*>(peer->data)->level >= 5) {
		level5 = "`oUnlocked: 2 Growtokens``";
	}
	else {
		level5 = "`a(Locked) 2 Growtokens``";
	}
	if (static_cast<PlayerInfo*>(peer->data)->level >= 6) {
		level6 = "`oUnlocked: Earth Mastery (level 2) - Chance increase to 3%``";
	}
	else {
		level6 = "`a(Locked) Earth Mastery (level 2) - Chance increase to 2%``";
	}
	if (static_cast<PlayerInfo*>(peer->data)->level >= 7) {
		level7 = "`oUnlocked: 2 Growtokens``";
	}
	else {
		level7 = "`a(Locked) 2 Growtokens``";
	}
	if (static_cast<PlayerInfo*>(peer->data)->level >= 8) {
		level8 = "`oUnlocked: Flawless: 2% chance to decrease seed grow time when planting``";
	}
	else {
		level8 = "`a(Locked) Flawless: 2% chance to decrease seed grow time when planting``";
	}
	if (static_cast<PlayerInfo*>(peer->data)->level >= 9) {
		level9 = "`oUnlocked: Earth Mastery (level 3) - Chance increase to 4%``";
	}
	else {
		level9 = "`a(Locked) Earth Mastery (level 3) - Chance increase to 4%``";
	}
	...
	if (static_cast<PlayerInfo*>(peer->data)->level >= 11) {
		level11 = "`oUnlocked: Flawless (level 2) - Chance increase to 3%``";
	}
	else {
		level11 = "`a(Locked) Flawless (level 2) - Chance increase to 3%``";
	}
	if (static_cast<PlayerInfo*>(peer->data)->level >= 12) {
		level12 = "`oUnlocked: 10000 Gems``";
	}
	else {
		level12 = "`a(Locked) 10000 Gems``";
	}
	...
	if (static_cast<PlayerInfo*>(peer->data)->level >= 14) {
		level14 = "`oUnlocked: 4 Growtokens``";
	}
	else {
		level14 = "`a(Locked) 4 Growtokens``";
	}
...
inline void equip_item(ENetPeer* peer, int id, const int type) {
	...
	equip:
		...
		for (int i = 0; i < x; i++) {
			...
			case 1904:
			{
				static_cast<PlayerInfo*>(peer->data)->skinColor = 2;
				break;
			}
			case 11006:
			{
				if (!silent_equip) Player::OnConsoleMessage(peer, "`oMy soul is awakened!. (`oSoul Scythe mod added`o)");
				static_cast<PlayerInfo*>(peer->data)->effect = 465;
				break;
			}
			case 1204:
			{
			...
			case 1780:
			{
				if (!silent_equip) Player::OnConsoleMessage(peer, "`wLegendBot-009 Mode is now active!`o (LegendBot-009 Mode enabled)``");
			...
			case 2952: case 9510: case 9512: case 9520: case 9514: case 9496: case 9492: case 9488: case 3932: case 9448: case 9452: case 9508: case 9454: case 9456: case 9458: case 9430:
			{
...
inline void restore_player_name(WorldInfo* world, ENetPeer* peer) {
	try {
		...
		if (static_cast<PlayerInfo*>(peer->data)->rawName == world->owner && !mod(peer))
		{
...
string get_player_buffs(PlayerInfo* pData) {
	...
	if (pData->cloth_hair == 4748 || pData->cloth_hair == 4750 || pData->cloth_hair == 4746 || pData->cloth_necklace == 6260 || pData->cloth_hand == 9164)
	{
		int display_id = 6924;
		if (pData->cloth_necklace == 6260) display_id = 6260;
		if (pData->cloth_hand == 9164) display_id = 9164;
...
inline void LoadPlayerData(ENetPeer* peer) {
	...
	if (pData->haveGrowId && pData->rawName != "") {
		try {
			...
			int effect = j["effect"];
			bool showlocation = j["showlocation"];
			...
			if (guildname != "")
			{
				try
				{
					...
					{
						ifstream.close();
						ColoredText("Failed to load player guild data! (sendworld) caused by " + static_cast<PlayerInfo*>(peer->data)->rawName, "red");
				...
				{
					ColoredText("Failed to load player guild data! (sendworld) (exception) caused by " + static_cast<PlayerInfo*>(peer->data)->rawName + " guild name " + guildname, "red");
					enet_peer_disconnect_now(peer, 0);
				}
				catch (std::runtime_error&)
				{
					ColoredText("Failed to load player guild data! (sendworld) (runtime) caused by " + static_cast<PlayerInfo*>(peer->data)->rawName + " guild name " + guildname, "red");
					enet_peer_disconnect_now(peer, 0);
				}
				catch (...)
				{
					ColoredText("Failed to load player guild data! (sendworld) (...) caused by " + static_cast<PlayerInfo*>(peer->data)->rawName + " guild name " + guildname, "red");
					enet_peer_disconnect_now(peer, 0);
				}
			}
			...
			if (!pData->adminLevel == 5 && pData->isinv) pData->isinv = false;
			if (!mod(peer) && !pData->Subscriber && pData->canWalkInBlocks) pData->canWalkInBlocks = false;
...
inline void send_ban_panel(ENetPeer* peer, string extra) {
	PlayerInfo* pData = GetPeerData(peer);
	if (extra != "") extra += "<CR>";
	Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`4Banning Panel``|left|32|\nadd_textbox|`eUser: " + pData->lastInfo + "<CR>" + extra + "bBefore `4Banning `bmake sure you have the proof, before `4Banning!|\nadd_text_input|days|`4Days:|" + to_string(pData->last_ban_days) + "|9|\nadd_text_input|hours|`bHours:|" + to_string(pData->last_ban_hours) + "|9|\nadd_text_input|minutes|`2Minutes:|" + to_string(pData->last_ban_minutes) + "|9|\nadd_spacer|small|\nadd_text_input|reason|`eReason:|" + pData->last_ban_reason + "|30|\nend_dialog|ban_panel|Cancel|`4BAN|");
}

inline void send_mute_panel(ENetPeer* peer, string extra) {
	PlayerInfo* pData = GetPeerData(peer);
	if (extra != "") extra += "<CR>";
	Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`!Muting Panel``|left|32|\nadd_textbox|`eUser: " + pData->lastInfo + "<CR>" + extra + "`r<CR>`!Choose the mute amount and write the reason.|\nadd_text_input|days|`4Days:|" + to_string(pData->last_ban_days) + "|9|\nadd_text_input|hours|`bHours:|" + to_string(pData->last_ban_hours) + "|9|\nadd_text_input|minutes|`2Minutes:|" + to_string(pData->last_ban_minutes) + "|9|\nadd_spacer|small|\nadd_text_input|reason|`eReason:|" + pData->last_ban_reason + "|30|\nend_dialog|mute_panel|Cancel|`4MUTE|");
}

inline void send_curse_panel(ENetPeer* peer, string extra) {
	PlayerInfo* pData = GetPeerData(peer);
	if (extra != "") extra += "<CR>";
	Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`!Cursing Panel``|left|32|\nadd_textbox|`eUser: " + pData->lastInfo + "<CR>" + extra + "`r<CR>`!Choose the curse amount and write the reason.|\nadd_text_input|days|`4Days:|" + to_string(pData->last_ban_days) + "|9|\nadd_text_input|hours|`bHours:|" + to_string(pData->last_ban_hours) + "|9|\nadd_text_input|minutes|`2Minutes:|" + to_string(pData->last_ban_minutes) + "|9|\nadd_spacer|small|\nadd_text_input|reason|`eReason:|" + pData->last_ban_reason + "|30|\nend_dialog|curse_panel|Cancel|`4CURSE|");
}
